...
import { biconomyGaslessSwap } from '../utils/biconomy';
import { Zap } from 'lucide-react';

export function GaslessBiconomy({ account, provider, onRefresh }: any) {
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      const txHash = await biconomyGaslessSwap(provider, amountIn, deadline);
      setTxHash(txHash);
      alert(`Swap successful! Tx: ${txHash.slice(0, 10)}...`);
  ...
  return (
    <div className="bg-gray-800/50 rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-white font-semibold">Gasless Swap (Biconomy)</h3>
        <span className="text-xs bg-green-500/20 text-green-400 px-2 py-1 rounded-full">⚡ 0 Gas</span>
      </div>
      <div className="bg-gray-900 rounded-xl p-4 mb-4">
        <div className="flex justify-between text-sm text-gray-400">
          <span>You pay</span>
          <span className="text-green-400">0 ETH (Biconomy pays)</span>
        </div>
      </div>
      <div className="mb-4">
      ...
      >
        {loading ? 'Processing...' : 'Swap Gasless (Biconomy)'}
      </button>
      {txHash && (
