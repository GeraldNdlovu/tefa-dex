...
async function main() {
  ...
  // Approve Router to spend tokens (wait for each approval)
  const amount = ethers.parseEther("1000");
  console.log("\n📝 Approving Router...");
  const approveA = await tokenA.approve(routerAddr, amount);
  await approveA.wait();
  console.log("   TKA approved");
  const approveB = await tokenB.approve(routerAddr, amount);
  await approveB.wait();
  console.log("   TKB approved");

  // Add liquidity
  console.log("\n💧 Adding liquidity...");
  const addTx = await router.addLiquidity(tokenAAddr, tokenBAddr, amount, amount);
  await addTx.wait();
